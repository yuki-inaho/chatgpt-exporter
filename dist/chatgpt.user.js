...
(function(jszip, _zumer_snapdom) {
	...
	function sanitizeUrl(url, allowDataImage) {
		...
		if (/^data:/i.test(cleaned)) return allowDataImage && /^data:image\//i.test(cleaned) ? url : "";
		return url;
	}
	function sanitizeHastUrls(node) {
		if (!node || typeof node !== "object") return;
		if (node.type === "element" && node.properties) {
			const props = node.properties;
			if (node.tagName === "a" && typeof props.href === "string") props.href = sanitizeUrl(props.href, false);
			if ((node.tagName === "img" || node.tagName === "source") && typeof props.src === "string") props.src = sanitizeUrl(props.src, true);
		}
		if (Array.isArray(node.children)) for (const child of node.children) sanitizeHastUrls(child);
	}
	...
	function transformContent$2(content, metadata, postProcess) {
		switch (content.content_type) {
			...
			case "execution_output":
				if (metadata?.aggregate_result?.messages) return metadata.aggregate_result.messages.filter((msg) => msg.message_type === "image").map((msg) => `<img src="${escapeHtml(msg.image_url)}" height="${escapeHtml(String(msg.height))}" width="${escapeHtml(String(msg.width))}" />`).join("\n");
